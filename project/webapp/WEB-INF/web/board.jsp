...
<%
	out.println("board Session is : " + (String) session.getAttribute("id"));
...
<body>
	...
	<p>작성일 : <%=boardList.get(0).getPost_date() %></p>
  	<%
		if(isTrue){
	%>
		<a href="insert_password?post_number=<%=boardList.get(0).getPost_number() %>">삭제하기</a> <a href="print">뒤로 가기</a>
	<%
		} else{
	%>		
		<a href="print?userId=<%=userId %>">뒤로 가기</a>		
	<%
		}
	%>	  
